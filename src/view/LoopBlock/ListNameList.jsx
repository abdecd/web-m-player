...
function ListNameList({shown,setManaging,setMusicListLoading,style,listStyle}) {
    ...
    useEffect(() => {
        ...
        return webMusicListStorage.namesSub.subscribe(refreshFn);
    },[]);
    ...
    return <div style={{display: shown ? "block" : "none", height: "100%", ...style}}>
        { isEditing
            ? <EditList
                listData={nameList}
...
function NormalList({listData,setManaging,setMusicListLoading,currentListIndex,setIsEditing,listStyle}) {
    ...
    var deleteList = useCallback((_,elem,index) => {
        ...
        if (index<currentListIndex) {
            webMusicListStorage.setCurrentNameIndex(currentListIndex-1);
    ...
    return <BasicList innerRef={root} style={{ ...listStyle }}>
        { listData.map((elem,index) => (
            <ListItem key={elem.key}>
                <LeftItem
                    name={elem.name}
                    subName={elem.subName}
                    clickFn={ev=>selectList(ev,elem,index)}
                ...
                <RightBtn
                    ...
                    clickFn={ev=>deleteList(ev,elem,index)}
                    longClickFn={ev=>deleteAllList()}
                    style={{flexBasis: "40px"}}/>
            </ListItem>
        ))}
    </BasicList>
...
function EditList({listData,currentListIndex,setIsEditing,listStyle}) {
    const root = useRef();
