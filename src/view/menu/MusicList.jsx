...
export default function MusicList({listData,loading=false,style,innerRef}) {
    ...
    var playMusic = useCallback(async (_ev,elem) => {
        if (await webMusicManager.nextByObj(elem)) webMusicManager.play();
    },[]);

    var addAheadMusic = useCallback((_ev,elem) => {
    ...
    var addMusic = useCallback((_ev,elem) => {
    ...
    return <>
        {(editedListData.length==0 && loading) ? (
            ...
            // 留MusicBar位置
            <BasicList innerRef={innerRef} style={{ paddingBottom: '10px', ...style }}>
            {
                editedListData
                .map(elem => ({
                    firstName: elem.name,
                ...
                .map(elem => (
                    <ListItem key={elem.key}>
                        <LeftItem
                            ...
                            tags={/^(content|file|http:\/\/127\.0\.0\.1)/.test(elem.src)?[]:["online"]}
                        ...
                        <RightBtn
                            ...
                            longClickFn={_ev=>addAllToAheadList()}/>
